...
async def test_create_job_success(client: AsyncClient, user_headers: dict):
    ...
    body = resp.json()
    assert_success(body)
    assert_job_data(body["data"])
    assert body["data"]["name"] == "My first job"
    assert body["data"]["output_format"] == "jsonl"
    assert body["data"]["credits_reserved"] > 0
...
async def test_create_job_csv_format(client: AsyncClient, user_headers: dict):
    ...
    assert_success(body)
    assert_job_data(body["data"])
    assert body["data"]["output_format"] == "csv"
...
async def test_list_jobs_empty(client: AsyncClient):
    ...
    body = resp.json()
    assert_success(body)
    assert_job_list_data(body["data"])
...
async def test_cancel_already_failed_job(client: AsyncClient, user_headers: dict):
    ...
    if status == "failed":
        ...
        assert_error(resp.json(), "BAD_REQUEST")
